...
Let's talk a little more about evaluating how reliable a linear regression model is.  We will talk about first assessing how well the model fits the data, and then the effect size, or a measure of how much the model accounts for variability in the data.
...
1. **Is there evidence that at least one coefficient that is not 0?**
...
2. **Is there evidence that each individual coefficient is not 0?**
...
Looking at our `R` output above we see there are two $R^2$ statistics given.  The problem with $R^2$ is that adding more variables will always cause it to increase.  If we add more degrees of freedom to our model, more "wiggle room" if you will, then we can always make our model approximate the data more closely.  In fact, potentially, if we add enough variables we will perfectly match the data. **However**, that's not necessarily a good thing.  Remember, the data you have is a sample and our goal is not to create a model that perfectly predicts the sample, but rather on that is useful in predicting what will happen beyond the sample.  A model that perfectly matches the sample data might not be useful at all when looking beyond the sample -- we've too closely modelled the randomness in the data.  This is called overfitting the model.

So we want not too many, and not too few predictor variables.  To balance the tendancy of $R^2$ to increase as we add more variables we use the adjusted-$R^2$.  You saw the formula in your reading and hopefully you noticed the formula takes into account the number of predictors we are using relative to the number of data points we have.
...
Remember, adjusted-$R^2$ tells us what percent of the variation in `Sales` is explained by the model.  Generally, a higher $R^2$ is better, but a low $R^2$ does not mean we should throw our model away.
...
For our example above, where we had adjusted-$R^2$ of 0.8564, we can say this is a high $R^2$, so I would use this model to make predictions.  However, since we saw that `Newspaper` is not significant, we could likely improve our model by removing it.  We'll talk more about that idea soon.
...
The adjusted $R^2$ for the model is 0.4938.  This inidcates that less than 50% of the variation in `price` is accounted for in this model.  That is not encouraging, so I would not use this model for any sort of precise predictions.  However, since each variable is significant, we can interpret their coeffcients.

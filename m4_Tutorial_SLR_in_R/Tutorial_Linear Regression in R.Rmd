...
Looking across the plot we see the average is close to 0 and no excessively outlying points.  A clear pattern is not evident.  So #1 is satisfied.  The vertical spread of the data is perhaps a little less on the left hand side, but not enough to worry about, so that supports #4.  This is good!
...
### Checking #2
Here we need to ask ourselves if it is reasonable to assume the data are independent. Presumably the data come from sales of unrelated clocks, so this assumption is met.
